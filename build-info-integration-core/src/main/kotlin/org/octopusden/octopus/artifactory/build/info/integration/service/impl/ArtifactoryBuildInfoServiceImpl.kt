...
class ArtifactoryBuildInfoServiceImpl(
        ...
        dependencies
            .flatMap { dependency ->
                ...
                try {
                    ...
                    logger.warn(
                        "Build info for dependency build ${dependency.buildName}:${dependency.buildNumber} not found. Skipping: ${e.message}",
                    )
                    emptyList()
                } catch (e: ArtifactoryClientException) {
                    logger.warn(
                        "Error retrieving build info for dependency build ${dependency.buildName}:${dependency.buildNumber}. Skipping: ${e.message}",
                        e,
                    )
                    emptyList()
                }
    ...
    override fun mergeBuildInfo(
        mavenBuildInfo: BuildInfo,
        npmBuildInfo: BuildInfo?,
        npmDependenciesModules: List<Module>,
    ): BuildInfo {
        if (npmBuildInfo != null) {
            logger.info(
                "Merging NPM build info (${npmBuildInfo.name}:${npmBuildInfo.number}) into Maven build info (${mavenBuildInfo.name}:${mavenBuildInfo.number})",
            )
        }
        ...
        if (npmDependenciesModules.isNotEmpty()) {
            logger.info(
                "Merging ${npmDependenciesModules.size} NPM dependency modules into Maven build info (${mavenBuildInfo.name}:${mavenBuildInfo.number})",
            )
