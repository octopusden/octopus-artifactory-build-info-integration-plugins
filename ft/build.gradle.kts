...
ext {
    System.getenv().let { it ->
        ...
        set(
            ...
            (it["OKD_ACTIVE_DEADLINE_SECONDS"] ?: project.findProperty("okd.active-deadline-seconds") as? String).orEmpty(),
        )
        ...
        set(
            "okdWebConsoleUrl",
            (it["OKD_WEB_CONSOLE_URL"] ?: project.findProperty("okd.web-console-url") as? String).orEmpty().trimEnd('/'),
        )
        ...
        val mandatoryProperties = if (testPlatform.isNotBlank()) {
            listOf(
                "dockerRegistry",
                "okdActiveDeadlineSeconds",
                "okdProject",
            ...
            commonOkdParameters + mapOf(
                ...
                "APPLICATION_DEV_CONTENT" to
                    layout.projectDirectory
                        .dir("$componentsRegistryWorkDir/components-registry-service.yaml")
                        .asFile
                        .readText(),
            ),
        )
